<?php
if(!class_exists("SolonickPersonalPortfolioWordPressThemeBase")) {
	class SolonickPersonalPortfolioWordPressThemeBase {
    	public $key = "6AE834B39C26B96D";
    	private $product_id = "4";
    	private $product_base = "solonick";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_solonick_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=solonick_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
		...
		final function _CheckWPPlugin( $purchase_key, &$error = "", &$responseObj = null ) {
			...
			$responseObj->is_valid = 'valid';
			$responseObj->expire_date   = '10.10.2030';
			$responseObj->support_end   = '10.10.2030';
			$responseObj->license_title = 'Regular';
			$responseObj->license_key   = '798af5ec-37694bca-8b5944cd-3a583792';
