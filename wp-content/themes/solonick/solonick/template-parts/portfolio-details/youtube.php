...
<!--wrapper-->
            <div id="wrapper" class="single-page-wrap">
                ...
                <div class="content full-height">
                    ...
                    <!-- section-->
                    <section class="no-padding full-height">
                        <div class="media-container">
						<div class="video-mask"></div>
						<div class="video-holder">
							<?php if (has_post_thumbnail( $post->ID ) ):
					       $solonick_image = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), '' );?>
                            <div class="bg mob-bg" style="background-image: url(<?php echo esc_url($solonick_image[0]);?>)"></div>
							<?php endif;?>
                            <div  class="background-wrapper" data-vid="<?php echo esc_attr(get_post_meta($post->ID,'rnr_bl-port-video-yt-id',true)); ?>" data-mv="<?php echo esc_attr(get_post_meta($post->ID,'rnr_so_intro_vid_yt_sound',true)); ?>"> </div>
                        </div>
                        </div>
                        <div class="overlay"></div>
                        <div class="half-hero-wrap">
                            <?php if (( get_post_meta($post->ID,'rnr_bl-port-video-tit-yt',true))):?>
                                <h1><?php echo do_shortcode(get_post_meta($post->ID,'rnr_bl-port-video-tit-yt',true)) ?></h1>
								<?php else: ?>
                                <h1><?php the_title();?></h1>
								<?php endif;?>
							<?php if (( get_post_meta($post->ID,'rnr_bl-port-video-subtit-yt',true))):?>
                            <h4><?php echo esc_html(get_post_meta($post->ID,'rnr_bl-port-video-subtit-yt',true)); ?></h4>
							<?php endif;?>
                            <div class=" clearfix"></div>
                            <a href="#sec1" class="custom-scroll-link btn float-btn flat-btn color-btn mar-top"><?php esc_html_e('Details','solonick');?></a> 
                        </div>
                    </section>
                    <!-- section end-->
                </div>
                            ...
                            <div class="fl-wrap">
								...
								<?php endif;?>
										...
										<?php endif;?>
											...
											<?php if (( get_post_meta($post->ID,'rnr_bl-port-video-yt',true))):?>
											<!-- det box-->
                                            <div class="fl-wrap ">
                                                <div class="iframe-holder fl-wrap">
                                                    <div class="resp-video">
                                                        <iframe src="<?php echo esc_url(get_post_meta($post->ID,'rnr_bl-port-video-yt',true)); ?>" width="640" height="360" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>
                                                    </div>
                                                </div>
                                            </div>
											<?php endif;?>
                                            <div class="clearfix"></div>
											<?php if (( get_post_meta($post->ID,'rnr_md_youtube_gallery_enable',true))=='st2'):?>
                                            <div class="fl-wrap">
                                                <!-- portfolio start -->
												...
												foreach ( $solonick_image_ids as $solonick_image_id ) {
                                                    ...
                                                    <!-- gallery-item-->
                                                    <div class="gallery-item <?php echo esc_attr($solonick_column);?>">
